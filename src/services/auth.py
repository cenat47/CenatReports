...
from src.exceptions import ExpiredTokenException, InvalidTokenException, ObjectAlreadyExistsException, UserAlreadyExists, UserNotFoundExc
from schemas.auth.user import LastLoginUpdate, User, UserAdd, UserLogin, UserRequest
...
class UserService(BaseService):
    ...
    async def authenticate_user(self, data: UserLogin):
        ...
        if db_user and is_valid_password(data.password, db_user.password_hash):
            await self.db.user.edit(data=LastLoginUpdate(last_login_at=datetime.now(timezone.utc)), id=db_user.id)
            return db_user
        return None
    ...
    async def create_token(self, user_id: uuid.UUID, ip_address: str) -> Token:
        ...
        await self.db.refresh_token.add(RefreshTokenAdd(
            user_id=user_id,
