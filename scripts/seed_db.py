...
async def create_report_templates(session):
    ...
    reports = [
        (
            "daily_sales",
            "Ежедневный отчёт по продажам. Пользователь вводит начальную и конечную даты. Отчёт показывает суммарную выручку, количество заказов, средний чек и другие метрики по каждому дню по дате создания заказа в указанном диапазоне, а также сумму всех платежей по этим заказам.",
        ),
        (
            "sales_by_categories",
            "Отчёт по продажам по категориям товаров. Пользователь вводит период (начальная/конечная дата) и при желании выбирает конкретную категорию. Отчёт выводит количество заказов и проданных единиц, суммарную выручку, средний чек по заказам и другие метрики по категориям за выбранный период.",
        ),
        (
            "sales_by_products",
            "Отчёт по продажам отдельных продуктов. Пользователь вводит период (начальная/конечная дата) и может выбрать конкретный продукт. Выводится количество проданных единиц, суммарная выручка по продукту, средняя цена/выручка на единицу и другие метрики по каждому продукту за период.",
        ),
        (
            "customers",
            "Отчёт по клиентам за период. Пользователь вводит начальную и конечную даты, а также выбирает режим: показать всех клиентов или только топ клиентов по сумме покупок. Метрики: количество заказов клиента, суммарная сумма покупок, средний чек и базовые показатели активности за период.",
        ),
        (
            "payments",
            "Отчёт по платежам за период. Пользователь вводит начальную и конечную даты и выбирает режим: все платежи или топ по сумме. Метрики: общая сумма платежей, количество транзакций, средний платёж и распределение сумм по способам оплаты за выбранный период.",
        ),
    ]
